...
def main(SN_NAME):
    ...
    # If you want to do again the LC fit call the function self.LCfit_withGP() again.""")
            ...
            fit_result2mangling.drop(index = 0,inplace = True)
            fit_result2mangling.to_csv(self.results_mainpath + 'fitted_phot4mangling_%s.dat' % self.snname, sep='\t',
                                       index=False)
            print("____________________________WRITING_________________________________________")
            print(fit_result2mangling)
    ...
    SN.LCfit_withGP(minMJD = 58855) #Filip, for 2020oi
