...
def get_grizXY(d):
    ...
    for j in range(len(snid_list)):
        ...
        generateList(g,r,i,z,x,y,snid_list[j])

def generateList(g,r,i,z,x,y,SNname):
    allObs = [g,r,i,z,x,y]
    result = pd.concat(allObs)
    result = result.sort_values(by=["MJD"])
    print(result)
    Inst = {    "g":"PAN_STARRS1",
                "r":"PAN_STARRS1",
                "i":"PAN_STARRS1",
    ...
    with open("SN" + SNname + "_mag.dat", 'w') as f:
        print("MJD,Mag,Mag_err,band,Instr",file = f)
        for i in range(len(result)):
            rowString = str(result.iloc[i]["MJD"]) + ","
            rowString += str(result.iloc[i]["MAG"]) + ","
            rowString += str(result.iloc[i]["MAGERR"]) + ","

            filt = result.iloc[i]["FLT"]
            inst = Inst[filt]
            band = Band[filt]
...
get_grizXY('./toGenerate')
#generateList(g,r,i,z,x,y)
