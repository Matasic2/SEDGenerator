...
def read_YSE_ZTF_snana_dir(dir_name,filename,keep_ztf=True):
    """
    file_path : str
    ...
    for file_path in sorted(glob.glob(dir_name+'/*')):
        ...
        if (file_path[:-10].endswith(filename) == False):
            continue
...
def plotFiles(g,r,i,z,x,y,snid,axis):
    allObs = [g,r,i,z,x,y]#[g,r,i,z,x,y]
    colorsList = [u"#4daf4a", u"#e41a1c", u"#832db6",u"#656364",u"#b9ac70",u"#bd1f01"]
    nameFilt = ["YSE-g","YSE-r","YSE-i","YSE-z","ZTF-g","ZTF-r"]
    for obs, color,label in zip(allObs,colorsList,nameFilt):
        #mjds = obs['MJD'][(obs['MJD'] > 58850) & (obs['MJD'] < (58867+150))].to_numpy()
        #mags = obs['MAG'][(obs['MJD'] > 58850) & (obs['MJD'] < (58867+150))].to_numpy()
        mjds = obs['MJD'].to_numpy()
        mags = obs['MAG'].to_numpy()
        magserr = obs['MAGERR'].to_numpy()
        #plt.plot(mjds,mags)
        axis.errorbar(mjds, mags,magserr, color = color, fmt='o', ms=7, elinewidth=2,label = label)
        #axis.scatter(mjds,mags,color = color,label = label)
        
def get_grizXY_plot(d,filename,axis):
    snid_list, meta_list, yse_ztf_fp_df_list = read_YSE_ZTF_snana_dir(d,filename)
    for j in range(len(snid_list)):
        ...
        plotFiles(g,r,i,z,x,y,snid_list[j],axis)

#get_grizXY_plot('./toGenerate',axis)
